...
$id_multivalue = $_GET['id_multivalue'] ?? null;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  // Validasi sederhana
  if ($jumlah <= 0) {
    header("Location: edit.php?id_multivalue=$id_multivalue&id_transaksi=$id_transaksi&error=invalid_jumlah");
    exit();
  }
      ...
      <header class="z-10 py-4 bg-white shadow-md dark:bg-gray-800">
        <div class="container flex items-center justify-between h-full px-6 mx-auto text-purple-600 dark:text-purple-300">
          <div class="flex justify-start flex-1 lg:mr-3">
            <a href="dashboard.php" class="transition duration-300 hover:text-white">Kembali ke laman dashboard</a>
          </div>
        </div>
      </header>
      ...
      <main class="h-full pb-16 overflow-y-auto">
        <div class="container grid px-6 mx-auto">
          <h2 class="my-6 text-2xl font-semibold text-gray-700 dark:text-gray-200">
            Formulir Edit Sampah
          </h2>

          <div class="container grid px-6 mx-auto">
            <form method="POST" action="edit.php?id_multivalue=<?= $id_multivalue ?>&id_transaksi=<?= $id_transaksi ?>" class="w-full max-w-4xl px-4 py-3 mx-auto mb-8 bg-white rounded-lg shadow-md dark:bg-gray-800">
              ...
              <label class="block mt-4 text-sm">
                ...
                <textarea
                  name="keterangan"
                  class="block w-full mt-1 text-sm dark:text-gray-300 dark:border-gray-600 dark:bg-gray-700 form-textarea focus:border-purple-400 focus:outline-none focus:shadow-outline-purple"
              ...
              <div class="flex justify-center mt-4 text-sm">
                <button
                  type="submit"
                  class="w-full px-4 py-2 text-sm font-medium leading-5 text-white bg-purple-600 border-transparent rounded-md max-w-3xs hover:bg-purple-700 focus:outline-none focus:shadow-outline-purple">
                  Simpan Perubahan
                </button>
              </div>
            </form>
